...
@export var is_fox: bool = false  # Determines if the entity is a fox
...
func reproduce(mate: Node3D):
	if simulation_manager and simulation_manager.has_method("add_entity") and mate.reproduction_timer <= 0 and reproduction_timer <= 0:
		# Set cooldown before spawning the new rabbit
		...
		new_animal.reproduction_timer = reproduction_cooldown  # Ensure the newborn cannot instantly reproduce
		...
		# Reset target to avoid immediate mating loop
		target = null
		mate.target = null
