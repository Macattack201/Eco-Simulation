...
func flee_from(threat_position: Vector3):
	...
	if new_position.x == 0 or new_position.x == grid_size - 1:
		new_position.z += randi() % 3 - 1
	if new_position.z == 0 or new_position.z == grid_size - 1:
		new_position.x += randi() % 3 - 1
...
func wander():
	...
	if (new_position.x == 0 or new_position.x == grid_size - 1) and x_offset == 0:
		x_offset = randi() % 3 - 1
	if (new_position.z == 0 or new_position.z == grid_size - 1) and z_offset == 0:
		z_offset = randi() % 3 - 1

	new_position = position + Vector3(x_offset * move_speed, 0, z_offset * move_speed)
	position = new_position
